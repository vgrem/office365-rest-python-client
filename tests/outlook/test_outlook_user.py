...
class TestOutlookUser(GraphDelegatedTestCase):
    ...
    @requires_delegated("Mail.Read", "Mail.Read.Shared", bypass_roles=["Exchange Administrator", "Global Administrator"])
    def test4_get_mail_tips(self):
        result = self.client.me.get_mail_tips([test_user_principal_name]).execute_query()
        self.assertIsNotNone(result.value)

    @requires_delegated("MailboxSettings.ReadWrite", bypass_roles=["Exchange Administrator", "Global Administrator"])
