...
class TestView(SPTestCase):
    ...
    target_field: FieldMultiLineText | None = None
...
<View>
  ...
  <RowLimit>100</RowLimit>
    ...
    def test3_get_view(self):
        ...
        result = self.target_view.get().execute_query()
        self.assertIsNotNone(result.resource_path)

    def test4_render_as_html(self):
        assert self.target_view is not None
        result = self.target_view.render_as_html().execute_query()
    ...
    def test6_get_view_items(self):
        assert self.target_view is not None
        result = self.target_view.get_items().get().execute_query()
        self.assertIsNotNone(result.resource_path)

    def test7_update_view(self):
        assert self.target_view is not None
        title_updated = self.target_view.properties["Title"] + "_updated"
    ...
    def test8_get_view_fields(self):
        assert self.target_view is not None
        result = self.target_view.expand(["ViewFields"]).get().execute_query()
        self.assertIsNotNone(result.view_fields)
        self.assertIsInstance(result.view_fields, ViewFieldCollection)
        type(self).view_fields_count = len(result.view_fields)
    ...
    def test_14_delete_view(self):
        assert self.target_view is not None
        view_to_delete = self.target_view
