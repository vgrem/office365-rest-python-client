...
class ChangeLogItemQuery(ClientValue):
    ...
    Args:
        ...
        query (str): Specifies which records from the list are to be returned and the order in which they will be returned. See section 2.2 in [MS-WSSCAML].
        query_options (str): Specifies various options for modifying the Query (section 3.2.5.183.1.1.3). See [MS-LISTSWS] section 2.2.4.4.
        contains (str): Specifies a string representation of the XML element that defines custom filtering for the query. See [MS-LISTSWS] section 2.2.4.3.
