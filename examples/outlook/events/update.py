"""
Update an existing event (reschedule, change subject, add attendees).
...
event = events[0]
event.set_property("subject", f"{event.subject} (updated)").update().execute_query()
print(f"Event updated: {event.subject}")
