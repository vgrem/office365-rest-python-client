"""Upload a pandas DataFrame to a SharePoint document library as CSV or XLSX.

``Folder.upload_dataframe`` serializes the frame and uploads it. CSV is written
UTF-8 with a BOM (``utf-8-sig``) so Excel opens it with the columns intact; pass
``--format xlsx`` for a worksheet (requires the ``[excel]`` extra).
...
def main():
    ...
    file = folder.upload_dataframe(args.name, df, format=args.format).execute_query()
