"""
Revoke a role from a user or group on a site, list, folder, or file.
...
def main():
    parser = argparse.ArgumentParser(description="Revoke a role from a user/group")
    parser.add_argument(
        "--scope",
    ...
    parser.add_argument("--role", choices=sorted(ROLES), default="contributor", help="role to revoke")
    ...
    target.remove_role_assignment(args.principal, ROLES[args.role]).execute_query()
    print(f"✓ Revoked '{args.role}' from {args.principal} on {args.scope}")
