...
def main():
    parser = argparse.ArgumentParser(description="Create a site design")
    parser.add_argument("--script-title", default="Contoso theme script", help="site script title")
    parser.add_argument("--design-title", default="Contoso site", help="site design title")
    ...
    script_result = SiteScriptUtility.create_site_script(
        ctx, args.script_title, "Applies the Contoso branding theme", site_script
    ).execute_query()
    ...
    design_info = SiteDesignCreationInfo(
        ...
        SiteScriptIds=ClientValueCollection(uuid.UUID, [uuid.UUID(script_id)]),
    )
